...
이 저장소는 **키워드 → 논문 드래프트** 자동화 파이프라인(`/paper` 명령)을 호스팅합니다. 본 문서는 메인 세션(오케스트레이터)이 따라야 할 규칙만 다룹니다. agent별 상세는 `.claude/agents/*.md`에 있고, 라우팅 카탈로그는 `.claude/_index.md`, 공통 규약은 `.claude/_common.md`에 있습니다.
...
- 어떤 작업이 들어와도 **agents/*.md 본문을 전수 Read하지 말 것**. 라우팅은 `.claude/_index.md` 표 1회 Read로 충분.
...
- `/paper <키워드> [auto]` — 전체 파이프라인. `.claude/commands/paper.md` 참조.
