...
2. **Rank-resolved latent-taxonomy probe (RQ2).** The same geometric audit replicated across the seven Linnaean ranks and across two backbones --- the biology-pretrained BioCLIP2 ViT-L/14 and the general-domain OpenCLIP ViT-L/14 --- with a random-taxonomy permutation control (per backbone) to test whether the image embeddings already encode partial hierarchical structure *independent of the text-side prompt*.
3. **Counterfactual prompt ablation (RQ3, central instrument).** Five prompt conditions C0-C4 designed to factor *structural alignment with the taxonomy* away from *lexical content*. We report effect-preservation ratios with bootstrap 95% confidence intervals, falling back to raw drops in the saturated-species regime (Section 3.1).

A cross-dataset replication (RQ4, partial) extends 1--3 from the single-class CUB-200 set (all 200 species are Aves) to a multi-clade iNaturalist-2021 evaluation subset (821 species, 8{,}210 images spanning five classes --- Aves, Insecta, Reptilia, Plantae, Fungi --- across three kingdoms), which removes the rank-homogeneity confound of CUB-200 and makes all seven Linnaean ranks non-degenerate. The full five-domain meta-analysis (Aves, Insecta, Plantae, Fungi, Actinopterygii) is identified as the remaining external-validity step.
...
- **Zero-shot top-1 accuracy** (image-text): each species prompt under condition $C_k$ is encoded to a class prototype $w_c^{(k)}$ and every image is assigned to $\argmax_c \, z_i^\top w_c^{(k)}$; accuracy is reported per condition. This is the only *cross-modal* metric and complements the image-only geometry by exposing whether prompt manipulations that leave the image geometry intact nonetheless degrade the text-side prototype.
...
**Pre-registration.** The hypothesis dichotomy, the five prompt conditions, the geometric and taxonomic metrics, the primary ratio-based decision rule, and the secondary raw-drop + rank-sign decision rule of Section 3.1 were fixed *before* execution and are mirrored without modification in `02_rqs.md`. The iNaturalist-2021 cross-dataset run was added to the protocol *after* CUB-200 results made the denominator-sign issue empirically clear, but it executes the same fixed measurement pipeline; we flag this as a *replication* step rather than a hypothesis-generating one and treat the multi-clade result as confirmatory only for the rank-sign pattern that CUB-200 already exhibited.
